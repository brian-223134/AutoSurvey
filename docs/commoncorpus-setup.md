...
**미결**: 섹션 수를 merge 단계까지 관철하는 파라미터화(§7 레버 표의 구조적 원인 해소).
논문 §2 는 "The outline predetermines the number of sections as 8"이라 명시하므로
merge 가 개수를 잃는 것은 구현 누락에 가깝다고 보지만, 프롬프트 문구를 건드리는
일이므로 **도입 여부는 별도 결정** (subsection_num 때의 "값 없으면 원문 그대로"
패턴을 따르면 원본 동작은 보존됨).
