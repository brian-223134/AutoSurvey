...
**서베이 7편 생성 완료** — 본편 4편 + 파이프라인 점검용 스모크 3편.
토픽당 `.md` / `.json` / `.tex` 3개가 나옵니다(스모크는 `.tex` 생략).

> 새 백본 `deepseek-v4-flash-0731`의 **본편은 생성 중**입니다(2026-08-05).
> 끝나면 이 표와 [`output/README.md`](output/README.md)에 한 줄씩 추가됩니다.
...
| `deepseek-v4-flash-smoke/` RAG for LLMs | **v4-flash-0731** | 5 / 41 | 44,803 | 268 | 486 | $0.17 |

7편 전부 `scripts/check_survey.py` 통과(댕글링 인용 0, json 매핑 일치).
haiku는 편당 6~8분. 결과 해석 시 주의점은 [`output/README.md`](output/README.md)에 있습니다 —
특히 **haiku 3편끼리만 통제된 비교**이고, v4-pro 편의 섹션 수는 `.md` 헤딩 기준이라 부풀려져 있습니다.

### 새 백본 스모크 — `deepseek-v4-flash-0731` (2026-08-05)

`--section_num 4 --outline_reference_num 200 --rag_num 15`, 배포본 DB. **축소 설정인데도
44,803단어**가 나왔습니다. 확인한 것과 배운 것:
...
| 출력 잘림 | ✅ 0건 |
| 파서 | ✅ 헤딩 레벨 오류·중복 없음 (v4-pro와 달리 `.md`와 `.tex` 기준이 일치) |
| 재시도 | ⚠ **429 5회** — Parasail 레이트리밋. 최종 실패는 0건(백오프로 전부 뚫림) |
| **분량 계수** | **1.56×** (지시 700 → 실측 1,093) |

**429는 `allow_fallbacks=false`의 대가입니다.** 붐빌 때 다른 provider로 넘어가지 않으므로
레이트리밋을 정면으로 맞습니다. 통제 실험을 위해 감수하는 것이고, `AUTOSURVEY_MAX_THREADS`를
낮춰 완화합니다(스모크 4섹션×4스레드=16 동시에서 5회. 본편 8섹션은 스레드를 2로).

> **v4-pro의 비용 실측이 예상의 2.3배였던 원인이 이 스모크로 확정됐습니다.** 모델이 아니라
> **라우팅**이었습니다. v4-pro는 엔드포인트 단가가 $0.435~$1.740로 4배 벌어져 있는데 그
> 실행에는 provider 핀이 없었습니다. 핀이 걸린 지금은 청구액이 단가표와 소수점까지 맞습니다.
...
| 산출물 | 섹션 | 서브섹션 | **섹션당 서브섹션** | 서브당 단어 | 지시값 대비 |
|---|---|---|---|---|---|
| haiku × 3편 | 8~9 | 48~51 | 6.0 | 531~537 | **0.76~0.77×** |
| deepseek-v4-pro | 10 | 72 | 7.2 | 1,167 | **1.67×** |
| **deepseek-v4-flash-0731** (스모크) | 5 | 41 | **8.2** | 1,093 | **1.56×** |

haiku·v4-pro는 `.tex` 기준, flash는 `.md` 기준입니다(헤딩 레벨 오류·중복이 없어 두 기준이
일치합니다 — v4-pro 편은 `.md`가 부풀려져 있어 `.tex`로 셌습니다).

**두 자유도가 곱해집니다.** 어느 쪽이 주범인지는 모델마다 다릅니다:

- haiku → v4-pro: 개수 1.41× / **길이 2.17×** → **길이**가 주범
- v4-pro → **flash**: 개수 1.14× / 길이 0.94× → 개수만 늘고 길이는 오히려 줄었습니다

즉 `deepseek-v4-flash-0731`의 **길이 계수 1.56은 v4-pro(1.67)보다 낮습니다.**
분량이 커지는 원인은 `"several subsections"`를 **섹션당 8.2개**로 읽는 데 있습니다.
그래서 이 모델에는 `--subsection_num`이 정확한 처방입니다 — 길이 하한을 건드리지 않고
분량만 줄일 수 있습니다.

> **`--rag_num`이 밀어올린 게 아닙니다.** 위 스모크는 서브섹션당 논문 **15편**만 주고도
> 1,093단어를 썼습니다. 입력량이 아니라 프롬프트의 하한 + `"comprehensive"` 프레이밍이
> 분량을 만듭니다.
...
**계수 실측 완료 (2026-08-05): 1.56×** — 스모크 결과는 §2. 이 모델은 길이보다 **개수**가
문제이므로(섹션당 8.2개) 처방이 다릅니다:

| 설정 | 예상 분량 | 판정 |
|---|---|---|
| 통제 없음 (`--section_num 8`) | ~71,700 (90p) | 초과 |
| **`--subsection_num 4`** — 길이는 700 그대로 | **~35,000 (44p)** | 광범위, ok |
| `--subsection_num 4 --subsection_len 450` | ~22,500 (28p) | 표준 |

**개수를 먼저 줄이세요.** `--subsection_num`은 아웃라인 단계에서 주제를 병합·탈락시키므로
남은 서브섹션의 밀도는 그대로입니다. `--subsection_len`을 내리면 **모든** 서브섹션이
얕아집니다. 서브섹션당 1,093단어는 SurveyForge(627)·haiku(535)에 비해 이미 넉넉합니다.
